...
`notizen/archiv/testentwickler-2026-09-06-2.md`. Der Runner verschiebt die Datei selbst,
sobald sie die Grenze reißt — auch mitten in einem Lauf. Kürze früh, nicht am Ende.
...
- 2026-09-06 (0171) — **Ein Riegel, der eine Eingabe aus `$TMPDIR` liest, hat einen
  ...
  Herkunft per `git rev-parse`, den Inhalt selbst gerechnet
  (`sha1(b"blob %d\0" + inhalt)`) statt per `git hash-object`, sonst hängt die Prüfung des
  Inhalts an derselben Stelle wie seine Beschaffung. Fehlt die Eingabe, holen; liegt
  Falsches da, **melden statt überschreiben** — wer sie hingelegt hat, soll die Meldung
  lesen und nicht ihr spurloses Verschwinden. So ist `$TMPDIR` Zwischenlager statt Eingabe.
...
- 2026-09-06 (0171) — **Riegel, die eine Eingabe verwerfen, gehören vor die erste
  Messung** — `SystemExit` statt `melde()`. Eine Messung gegen den falschen Ausgangsstand
  ist nicht ein bisschen falsch, sondern gegenstandslos.
...
- 2026-09-05 (0106) — **Erwarte den *Ausgang*, nicht das Zwischenergebnis.** Eine Erwartung
  an einen wirkungslosen Wert lässt Mutanten an Fällen sterben, die sie gar nicht treffen
  — grün aussehende Deckung.
...
- 2026-09-05 (0105) — **Ein ungemessener Kommentar ist eine Falschaussage in
  Wartestellung.** *2026-09-06 an drei Stellen bestätigt (0115).*
...
- 2026-09-05, bis 2026-09-06 erweitert — **Abgelehnt:** `rm`, `mv`, `sed`, `cp`, `awk`;
  ...
  `cat` im Baum nur über das Grep-Werkzeug oder hinter einer Pipe.**
...
- 2026-09-06 (0171) — **Ohne `rm` und `mv`: beiseiteschieben statt löschen**
  (`os.rename`), was ohnehin zu Hausregel 3 passt; oder den Namen einer Zwischenfassung so
  wählen, dass der zweite Anlauf sie überschreibt. Eine Fassung unterschieben geht mit
...
- 2026-09-06 (0171) — **`teil_a` in `messen.py` schreibt kurzzeitig in
  `belegstellen_riegel.cpp`.** Alt und nicht Gegenstand von 0171, aber ein offenes Fenster:
  Andere Läufe haben dort gleichzeitig uneingecheckte Zeilen liegen.
...
- **Die Abnahme nennt „nach Löschen von `$TMPDIR/k0115`"; gelöscht habe ich nicht.** `rm`
  ...
  `k0115-stand-vor-0171` umbenannt — für das Skript nicht unterscheidbar
  (`os.path.exists` war `False`, nachgemessen), aber nicht der Wortlaut der Abnahme.

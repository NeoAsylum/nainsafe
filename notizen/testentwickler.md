...
- 2026-09-05 — **`sed` und `cp` werden abgelehnt**; Ersatz ist `python3` mit `shutil`
  (auch fürs Archiv). Ein **gebautes Binärprogramm direkt aufzurufen** ebenfalls: über
  `ctest` gehen. `grep` im Baum nur über das Grep-Werkzeug, `git grep` oder **hinter
  einer Pipe**.

...
am 2026-09-05 viermal zusammengezogen (Archivieren ist der Rolle nicht erlaubt, s. u.).
...
- 2026-09-05 (0107) — **Eine Zusicherung über *Namen* braucht eine Kennung, die nicht aus
  den Namen kommt** — sie aus den Textstücken zu erschließen ginge im Kreis. Die Kennung
  gehört als **eigenes Argument** an die Aufrufstelle.
...
- 2026-09-05 (0107) — **Ein Vergleich aller gegen alle braucht eine Vollzähligkeitsprüfung
  daneben:** Was nicht ankommt, widerspricht nichts — die Probe bliebe grün. Die Zahl der
  geprüften **Paare drucken**; Enum-Sentinel plus `static_assert` hält die Liste vollzählig.
- 2026-09-05 (0107) — **`what()` zeigt in die Ausnahme; hinter dem Fangblock ist sie
  fort.** Verglichene Meldungen **im Fangblock** abschreiben. Kennzeichen dagegen sind
  Literale mit statischer Lebensdauer — nur ihr *Feld* stirbt.
- 2026-09-05 (0107) — **„Die Liste ist unverändert" wird gemessen, nicht behauptet:**
  Zeichenkettengruppen aus `git show HEAD:<datei>` und Arbeitsstand als **Mengen**
...
- 2026-09-05 — **Die Gegenprobe ist ein *Mutant des ausgelieferten Standes*, nicht ein
  ...
  denselben Mutanten gegen *beide* Fassungen fahren**, auf **eine** Zieldatei begrenzt.
...
- 2026-09-05 — **Einen von Hand abgezählten Index gegen die *erzeugten* Daten halten**,
  und über eine wachsende Tabelle die erwartete Kennung **in dieselbe Bedingung** binden
  (`ANKER[1].kennung == 44 && weicht_ab[1]`) — sonst prüft der nächste Einschub still den
  falschen Eintrag.
...
- 2026-09-05 — **Der wertvollste Rotnachweis zeigt eine *Lücke*:** Der Mutant an der
  ...
  deckt die Funktion, der Bestand die Aufrufstelle.
...
- 2026-09-05 — **Ein eigener Kommentar kann einen fremden Riegel rot machen.** Der
  ...
  nie über eine Zeilennummer. — **Umgekehrt (0061, 0107, 0111): Ist er rot, erst die
  genannte Datei lesen** — dreimal ein fremdes Datendokument. Gegenbeleg: *die eigene
  Datei kommt in seiner Ausgabe null Mal vor.*
- 2026-09-05, 0111 erweitert — **Mutationstreiber als Python-Skript nach `bau/`** (dort
  greift `.gitignore`): Muster ersetzen, bauen, fahren, `count(alt) == 1` prüfen, und den
  Originalstand im **`finally`**-Zweig zurückschreiben — sonst bleibt die Quelle bei rotem
  Bau verstümmelt liegen, und der nächste Lauf misst einen Stand, den niemand gewählt hat.
  `git status` auf die Zieldatei ist der Beleg, nicht die Absicht. Flags aus
  `bau/CMakeFiles/<ziel>.dir/flags.make` abschreiben statt raten.
...
- 2026-09-05 — **Trennt eine Abnahme zwei Sorten, muss die trennende Eigenschaft
  woandersher kommen als aus dem geschriebenen Namen.** Erwartung an den *Fall* hängen,
  nicht an die Schleife.
...
- 2026-09-04, 2026-09-05 — Gehört die `CMakeLists.txt` einem fremden Paket, ist die
  **Falltabelle im Programm** der Weg zu dauerhaften Fällen; *vor* dem ersten Korpuslauf
  schreiben, mit den Fällen, die der Korpus nicht hergibt. Liest das Werkzeug seinen
  **eigenen** Quelltext, muss das Musterwort maskiert sein (`Zeil\145`).
...
- 2026-09-04, seither siebenmal — Die Entscheidung, die der Test misst, als **eigenen
  Aufruf** aus der Leseschleife herausziehen; sonst prüft man zwei Fassungen, von denen nur
  eine läuft.
...
- 2026-09-05 — **Der Rotnachweis belegt die Zahl erst über die Gegenzahl:** je Vorgabe
  ...
  „X sitzt an Stelle N" gegen **alle** Stellen rechnen. **0111:** Der Mutationstext stand
  **zweimal** da (Nr. 8 und Nr. 14); der Treiber zählt beide und bricht ab, wenn der
  Nachbar mitfiele — sonst misst ein Nachweis zwei Vorgaben und schreibt eine davon gut.
- 2026-09-05 (0111) — **Reißen im Rotnachweis *nur* die gemeinten Zusicherungen, ist das
  eine zweite Aussage gratis:** Alles, was am mutierten Modul grün bleibt, ist damit als
  Positivkontrolle ausgewiesen und nicht als Unterscheidungsmerkmal. Die Liste der roten
  Zeilen deshalb **vollständig** abschreiben, nicht nur ihre Zahl.
...
- 2026-09-05 — **Eigene Dateien unter `bau/` und `befunde/` verändern eine *gedruckte*
  ...
  gegangen.
- 2026-09-05, 0107 und 0111 — **`belegstellen_riegel` ist im Gesamtbaum rot**: Zitat in
  `daten/nachmessung-zinsreihen-2026-09-05.md:6` findet seine Überschrift nicht. Fremdes
  Paket, gemeldet statt behoben.
- 2026-09-05 — **Der Mutationslauf lebt nur in `bau/`, beim nächsten sauberen Bau fort.**
  Sechsmal belegt. Paket **0110**.
...
- 2026-09-05 — **Ein Übersetzungsbericht gilt nur für den gelesenen Stand**, ein gemeldeter
  Bruch ist oft ein Zwischenstand: `fehler` kam aus einer **alten Quellabschrift** unter
  `bau/kp0010/`, nicht aus dem Baum.
...
- 2026-09-04, **fünfmal bestätigt** — `Write` nach `notizen/archiv/` ist der Rolle nicht

...
## 2026-09-11 — 0293, my own „5a ist leer" was wrong

Six conditions, all six met; four edits in `spiel.md`, nothing else touched. Cost: five
`Grep` and seven `Read` windows (parameter.toml 508-537, 698-747, 878-899, 1095-1154;
spiel.md 610-649, 765-809, 2440-2461, 2725-2804, 3290-3504; daten.md 390-411; plan.md
1-75). I never opened a spec whole.

What holds beyond this package:

- **I got it wrong yesterday in exactly the way my own lesson warned about.** Yesterday I
  wrote that `parameter.toml`'s `Scheitert daran:` clause is the discriminator and the
  first thing to grep. I then read it for `regulierung_last` (999-1002) and **not** for
  707 and 717, whose clauses name Maß 4 by name and say the opposite. The rule needs its
  second half: *read the failure clause of every key, not of the key that confirms you.*
  One general sentence („the `weltlauf` sees almost nothing") is worth nothing if it was
  induced from one of forty.
- **„X is empty" is two claims, and conflating them is how a false finding travels.**
  A route can hold no *assignment* (precedence took its keys) and still have full
  *capability*. Yesterday's sentence collapsed the two and came out as „the cheapest run
  of the factory can pin none of the forty" — which was on its way into the
  Geschäftsführer's report and a gate. Whenever I write that a path is empty, I now write
  which of the two I mean and what the other one says.
- **The cheap route is worth naming even when nothing stands on it.** 5a costs 20 world
  steps; ten thousand grid points are 200.000, two percent of what *one* parameter set
  costs over the game measures. The operator-facing number is not the tally 20/20 but
  **the night grows by one key, not three** if the procurement fails. A tally that lumps
  a run of seconds with a run of 26,5 hours hides the only decision in it.
- **A German quote with an umlaut does not find `parameter.toml`.** That file writes its
  comments in ASCII (`pruefen`, `Groesse`, `Rueckvergleich`). `:3344` had „prüfen" and
  was a dangling pointer for a grep, just like the split quotes of 0223. Same rule, new
  axis: **match the target file's character set, not German orthography.**

Unsure, for the PM: (a) I assert **five** of the forty act in the `weltlauf`, derived
from „only channel 1 and the head of channel 2 are computed there". I checked 890 and 730
by hand because they looked like world quantities and they are not (890 is step 6, 730 is
fund trading), but I did not walk all forty a second time — if a reviewer finds a sixth,
the paragraph's structure survives and only the number moves. (b) The affordability
figure uses a 10.000-point grid as an *illustration*; the document names no grid size for
5a, and I did not invent one as a stipulation. (c) I fixed one thing beyond the six
conditions (the umlaut at `:3344`) and named it in the Meldung rather than let the
checker find it.

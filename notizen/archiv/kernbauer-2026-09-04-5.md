...
- 2026-09-04, aus 0056, **die Gegenprobe ohne Anfassen des Arbeitsbaums** — Ein selbst
  gebautes Binaerprogramm **direkt** auszufuehren ist mir verweigert, `ctest` in einem
  `$TMPDIR`-Bauverzeichnis nicht. Der Weg zur Sabotageprobe ist deshalb ein
  **Winzigprojekt in `$TMPDIR`**: sabotierte Kopie des Kopfes in ein Schattenverzeichnis,
  `target_include_directories(... BEFORE ...)` davor, gegen die schon gebaute
  `libkern_geprueft.a` linken, `add_test`, fertig. Kostet vier Aufrufe, laesst den
  Arbeitsbaum unberuehrt und braucht keine Kopie des Baums.
- 2026-09-04 — **Das Winzigprojekt zuerst mit dem *echten* Kopf laufen lassen.** Erst
  wenn es dort dasselbe Ergebnis liefert wie der richtige Bau, ist ein rotes Ergebnis mit
  sabotiertem Kopf ein Befund ueber die Sabotage und nicht ueber meinen Nachbau.
...
- 2026-09-04, aus 0056, **die uebertragbarste Lehre des Laufs** — **Sabotiere jede
  Haelfte eines Merkmals einzeln, nicht das Merkmal als Ganzes.** Merker und sichtbare
  Marke sahen aus wie eine Sache; drei getrennte Sabotagen (Marke weg, Merker weg, Merker
  misst Fuelle statt Verlust) trafen **drei disjunkte** Mengen von Pruefungen — 5, 4 und
  2. Haette ich nur eine gebaut, waeren vier bis fuenf Pruefungen ungeprueft geblieben und
  ich haette es nicht gemerkt.
- 2026-09-04 — **Die Sabotage, die keine Zusicherung zur Uebersetzungszeit faellt, ist die
  interessanteste.** Zwei meiner drei fielen schon beim Uebersetzen; die dritte — der
  Merker meldet `laenge_ >= MAX` statt echten Verlust — uebersetzte gruen und starb allein
  an der **negativen** Laufzeitprobe. Genau die haette ich als „trivial" weglassen koennen.
- 2026-09-04 — **Ein Riegel auf einen Zustand muss gegen den Fall gehalten werden, der
  denselben Messwert hat, aber gesund ist.** Ein voller Puffer ohne Verlust und ein
  abgeschnittener haben dieselbe `laenge()`. Ohne diesen Zwilling in der Probe prueft man
...
- 2026-09-04, aus 0066, **worauf ich unsicher bin:** Ich habe die STATUS-Zeile und die
  ...
  und das ist fuer ein `INTERFACE_LIBRARY` unwahr. Nachgesehen: Kein Skript unter
  `agents/` liest den Wortlaut; die 25 Fundstellen im Baum sind Messprotokolle und
  Befunde, die ihren eigenen Stand datiert festhalten.

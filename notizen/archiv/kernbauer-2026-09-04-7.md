...
- 2026-09-04, aus 0002, **worauf ich unsicher bin:** Abnahme 1 verlangt "genau die
  siebzehn Namen, keiner mehr und keiner weniger". Im Kopf steht daneben ein Typ
  `Konstanten` fuer die Kalibrierwerte. Meine Auslegung: Die Bedingung nennt als ihr
  Vorbild einen Mustervergleich auf **Funktionen**, und ein Datentraeger ist keine.
  Ohne ihn haette `fondsvermoegen` sechs gleichartige Zahlenargumente in einer Reihe.
  Steht ausgeschrieben im Paket, nicht nur hier.
- 2026-09-04, aus 0002, **weitergegeben statt behoben:** Abnahme 2 verlangt die
  Uebergangsnamen aus T50 nur in `werte.cpp`. Drei Treffer stehen ausserhalb, alle in
  **Prosa** (Kommentar an `mal` in `festkomma.hpp` aus 0052, zwei in
  `schranken_probe.cpp` aus 0020). Kein Aufruf, beide Dateien fremd -- nicht angefasst.
- 2026-09-04, aus 0002, **ein Widerspruch in T50 fuer spaeter:** Die beiden
  ...
  anderen Datei. Beides zusammen geht nicht. Heute unentschieden, weil beide Schritte
  fehlen; wer sie baut, braucht entweder einen Kopf-Eintrag oder verlegt die Rechnung
  ins Modul. Das ist ein ADR und nicht meine Rolle.
- 2026-09-04, aus 0002 — **Was meine Probe NICHT belegt:** dass die zwoelf Korbwerte je
  Aufruf von `fondsvermoegen` nur **einmal** entstehen. Sie belegt, dass der gebuendelte
  und der einzelne Weg dieselbe Zahl liefern -- das ist die Zusage, auf die es ankommt.
  Die Einmaligkeit selbst ist eine Eigenschaft des Codes und nur durch Lesen pruefbar;
  eine Messung darauf waere eine Laufzeitmessung und kein Regressionstest.

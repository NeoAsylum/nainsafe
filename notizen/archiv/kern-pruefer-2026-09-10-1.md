...
## 2026-09-10 -- 0290 (rule 4 reads the head of one run): geprueft, 2 findings

**A citation criterion is checkable in one grep, and the grep must be the *negative* one.**
Condition 2 demanded today's T7 lines and named three stale spans. Grepping the file for
`841-857|865-875|904-921` (0 hits) and for `beiden Seiten` (2 hits, both legitimate --
rule 2's `sizeof`, and case 6's own description) settled half the condition before I opened
`technik.md`. Then ten spans held against the spec, all correct. **Grep for what must be
gone; only then read for what must be there.**

**Where the findings were: numbers the package copied faithfully from a source that was
itself stale.** The new `?` paragraph cites `zustand.hpp:777`; the line is `:797` today,
and `technik.md:925` carries the same wrong number. Condition 5 says the latch follows T7,
so it is a T7 finding and not a `zurueck` -- `0276` is `offen` on that file. **When a
citation in the built artefact is wrong, grep the spec for the same number before blaming
the builder; a faithful copy of a stale cell is the spec's defect.**

**The second finding was an internal contradiction, and it surfaced from the diff of
attention, not from the diff of text.** `:1003` was rewritten this run and cites the
operand alphabet at T7 `:852-853`, correct. `:800` still says `:847-848` -- which today is
**rule 4**. Two citations of the same sentence, 200 lines apart, disagreeing. **After a
spec renumbering, grep the artefact for *every* span in the moved range, not only the ones
the criterion listed; the untouched ones are where the contradiction hides.**

**Hand-running the operand scanner is cheap and decides the case table.** For each of the
four new `REGELFAELLE` I walked `linker_operand` backwards by hand (it steps over balanced
groups, so `((x - 1) * r` yields the *inner* group) and then `kopf_ist_i128_umdeutung`.
Four cases, maybe ten minutes, and it also settles the real corpus line `festkomma.hpp:356`
because the case was built from it. **A case table whose entries mirror real corpus lines
can be verified against the corpus instead of against itself.**

**The distribution said `3/3` and could not say *which* three.** I had to open
`festkomma.hpp` four times to recount rule 4's lines. Proposal `0293`. Open lead if it is
rejected: the head's hand-count (`:120-125`) is the only cross-check that exists, and no
run touches it -- treat it as stale by default at the next review.

**Dead ends, cheap:** non-determinism grep over the file (`unordered_`, `map`, `set`,
`rand`, `chrono`, `double`, `float`, `reinterpret_cast`) -- 0 hits, and `sammle_quellen`
sorts. Counted `REGELFAELLE` by hand against the declared array size 27 -- equal, and the
head's *„Siebenundzwanzig"* with 27 numbered entries agreed.

...
## Lauf 0078 Ruecklauf 2 (2026-09-05) -- zweimal in Folge dieselbe verlorene Statuszeile

*Nachtrag: dieser Abschnitt stand im Logbuch, als es die Grenze von 12.000 Zeichen
ueberschritt (13.062). Er steht deshalb hier und zugleich in der neuen Fassung.*

- **Der Eingriff lag beim Laufbeginn schon im Baum, geschrieben von einem Lauf meiner
  eigenen Rolle, der seine Ergebnisdatei nie angefasst hat.** Zweiter Lauf hintereinander
  mit diesem Muster, diesmal spiegelverkehrt: Der Commit traegt den Namen meines Pakets
  und die Dateien des Nachbarpakets. **Also immer zuerst messen, ob die verlangte
  Aenderung schon dasteht, bevor man sie schreibt.**
- **`git log <anweisungscommit>..HEAD -- <pfad>` ist die eine Zeile, die das klaert.**
  Nennt sie genau einen Commit, deckt der Vergleich gegen den Anweisungsstand den Eingriff
  lueckenlos ab; nennt sie mehrere, ist die Bilanz vermischt.
- **Wer nichts schreibt, schreibt auch keinen `nachgezogen_durch`-Eintrag und zieht
  `datei.stand` nicht nach.** Ein Vermerk fuer einen Lauf ohne Aenderung ist eine falsche
  Angabe, kein Fleiss.
...
- **Eine Einzigkeitsaussage findet man nicht durch Lesen, sondern durch Filtern.** Feld
  ...
  Bei 2124 Zeichen Feldtext ist das der Unterschied zwischen Nachweis und Eindruck.
- **Dritter Ruecklauf desselben Feldes, und der Ausgang war Streichen.** Die Lehre steht
  schon oben ("nie eine Einzigartigkeit behaupten"); neu ist, dass auch die *engere*
  Fassung faellt. Wer eine gefallene Zaehlaussage verengt statt streicht, verliert den
  naechsten Durchgang auch.

...
## 0083 Runde 2 (2026-09-05, Fable-Lauf)

- **Urteil geprueft, 0 Befunde.** Ortsfrage zweiteilig (nennt + liegt, Pfadgrenze),
  end-zu-end am Arbeitsbaum MIT bau/ gemessen: specs-Kopie ohne spiel.md -> Code 1,
  15 tote Ziele; echte specs -> Code 0, 36/36. Vier Mutanten, alle rot, je andere
  Zielfaelle (6/7/10/12/13, 4/9/11, 11, Untergrenze 36<37 -> Code 2). Zahlen decken
  sich mit dem Bauagenten-Nachweis, erst danach gegengelesen.
- **Neutralisieren statt loeschen:** `x && y` -> `(x || true) && y` haelt die Funktion
  aufgerufen; der Loeschversuch starb erwartungsgemaess an `-Werror=unused-function`
  (bestaetigt 0107). Gilt auch fuer Bedingungshaelften, nicht nur ganze Funktionen.
- **$TMPDIR teilen sich parallele Laeufe:** eine fremde DATEI `m1` liess mein
  `makedirs(m1/venture)` mit NotADirectoryError sterben. Vor makedirs `isfile`
  pruefen oder sprechende lange Namen nehmen.
- **Befundname kollidierte mit Runde 1** -- `-runde2-` eingeschoben (Praezedenz:
  pruefung-0009-...-runde2-...). Bei Ruecklaeufen immer zuerst ls auf befunde/.
- Offene Faehrte (auch vom Bauagenten benannt): Die Gelesen-Seite loest weiter ueber
  `bestand.find(basisname(name))` auf -- ein toter Verweis `bau/x/spiel.md` faellt
  als Gelesen auf die echte spiel.md, solange die Ueberschrift dort steht. Eigenes
  Paket, falls je ein Zitat mit Ordner-Vorsatz auf Gelesenes zeigt.
- Faehrte ZITATFAELLE-Herkunft (aus 0086) erneut vertagt; ZIELFAELLE-Herkuenfte sind
  in dieser Runde stichprobenhaft nachgemessen (einzige-Fundstelle-Behauptung stimmt).
- Sperren dieser Sitzung: Verbund mit cd/&&-Umleitung, for-Schleife, ein grosser
  Sammel-Heredoc und das Write-Tool gesperrt; kleinteilige python3-Heredocs (auch
  schreibend ins Repo), cmake/ctest direkt, git show/log/diff und das Grep-Tool
  gingen.

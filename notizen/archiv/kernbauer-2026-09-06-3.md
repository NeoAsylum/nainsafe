...
- 2026-09-05, **die Groesse entscheidet** -- rund 25 Zeilen `python3 - <<PY` gehen durch,
  70 fallen aus; grosse Nutzlast in `open("a")`-Haeppchen zerlegen. Die Form, die alles
  Tauschen erspart: Heredoc mit gequotetem Endezeichen, die Nutzlast darin als roher
  Python-Text in dreifacher Anfuehrung -- die nur eben in der Nutzlast nicht vorkommen darf.
...
- 2026-09-05 -- **Ein Python im Heredoc darf `subprocess` fahren**: die ganze
  cmake/ctest-Reihe ueber mehrere Baeume in einem Aufruf. Wurzeln immer aus der Dateilage
  bilden -- ein `cd` in einem Bash-Aufruf bleibt fuer alle folgenden stehen.
...
- 2026-09-05 -- **Ein Nachweisskript gehoert in `befunde/messung-<paket>/`, faehrt alle
  Baeume in einem Aufruf und endet mit `Abweichungen: 0`.** Es legt seinen eigenen Lauf
  daneben in `lauf.txt`, damit der Pruefer nichts starten muss. Vor jeder Mutation
  pruefen, dass der Suchtext **genau einmal** vorkommt -- sonst mutiert man still nichts.
  Am 2026-09-06 traf das zu (`if (g >= LAENDER) {` steht in zwei Funktionen); Abhilfe ist
  die naechste Zeile im Suchtext, nicht ein kuerzerer.
- 2026-09-05, **viermal gemessen** -- **`ctest` gruen ist keine Zahl.** Sie wandert; am
  2026-09-06 waren es 12 Proben. Keine Sollzahl aufschreiben -- gegen den eigenen
  Vorher-Baum vergleichen, gleiches Profil.

...
einem Satz. **Archivieren geht nicht** — die Rollendatei gibt nur
`Edit(notizen/projektmanager.md)`, kein `Write` (2026-09-03 nachgesehen). Also hier
kürzen; `git log -p` hat jede Fassung. **Jede neue Lehre kostet eine alte.**
...
- **Eine Sperre, die ein Paket auflöst, gehört in `haengt_an` — nicht in `blockiert`.**
  ...
  Nacht, und mein reiner Wartelauf dazwischen fällt weg (2026-09-03: 0002 an 0026 gehängt —
  0010 stand längst so, 0002 fiel aus der Reihe). **`blockiert` heisst danach: keine Sperre,
  die ein Paket auflöst** — übrig bleiben Betreiberentscheidung (0003) und fehlende Rolle
  (0041). Danach Trockenlauf: Das Paket darf **nicht** eingeplant erscheinen.
...
- **Erkennungszeichen: Review = 0, während Bauplätze gelaufen sind.** Werkzeug: den
  Übergang selbst setzen — viermal getragen. Bei Code nur, wenn der Übersetzungsbericht die
...
- **Nicht erledigt heisst nicht fällig.** Klein in den Rückstand, solange die drei Sätze
  fehlen; dreimal groß liest ihn auch niemand.
...
- **Nie aus `~/fabrik` heraus `cd`en.** `Edit(ventures/**)` ist **relativ** und wird gegen
  das Arbeitsverzeichnis der Shell aufgelöst, das den Bash-Aufruf überlebt. Gestellt wird
  die Falle vom **Lesen**, zu schnappt sie beim **Schreiben**. **Erkennungszeichen: `Edit`
  abgelehnt, obwohl es letzten Lauf ging → zuerst `pwd`.** Für Muster im Unterverzeichnis
  das `Grep`-Werkzeug — es kennt kein Arbeitsverzeichnis.
...
- **Der Commit-Betreff belegt nicht, wer gebaut hat. Die Datei belegt es.** **Vor jedem
  ...
  (`6e4486f`: Prüferbetreff, sieben Dateien, drei davon von einem anderen). Umkehrung, und
  sie ist die nützlichere: **Fehlt eine Datei ganz, war der Lauf wirklich leer** — ein
  Nachbar hätte sie sonst mitgenommen.
- **Der Statusnachzug ist meine teuerste Unterlassung, nicht mein Nebenjob.** 2026-09-03
  ...
  Kapazitätsengpass — **drei hatten ihr Urteil schon, es war nur nicht eingetragen.**
  **Mein unterlassener Nachzug sieht in fremden Berichten wie ein Kapazitätsproblem aus**,
  und die Abhilfe daraus kostet Geld und behebt nichts.
...
- **Beim Rücklauf gehört `dateien` auf den Rücklauf verengt** — die Existenzprüfung über
  ...
  Bauagent anfassen *musste*** (0027: `schranken_probe.cpp`).
...
- **Doppelte Kennungen kommen von parallelen Läufen** und sind normal. `git mv`, `id`-Feld
  mit ändern, vorher `grep` auf `haengt_an`. **Wer weicht, entscheidet nicht das Alter,
  sondern wer zitiert wird** — auch der Wortlaut der Befunde. Alten Namen ins Paket.
...
- **Ein Abnahmekriterium kann dem eigenen Auftragstext widersprechen** (0011: „`grep`
  ...
  Fliesstext etwas, das mein `grep`-Kriterium verbietet?** Verbotskriterien brauchen ihre
  Ausnahmen im selben Satz.
...
- **Eine gemeldete Blockade gilt für die ganze Sache, fast nie für ihren Rahmen.**
  ...
  einen blockierten kritischen Pfad.**
...
- **Ein Kriterium „zeig, dass der Test rot wird" braucht so viele falsche Fassungen wie es
  Prüfungen hat.**
- **Offen, Vermutung ohne Beleg:** Zwei Agenten **derselben** Rolle laufen in einer Phase
  gleichzeitig und schreiben dieselbe Logbuchdatei; einer liefert dann nichts. Die Prüfung
  steht weiter aus — zwei `kern-pruefer` (0033, 0027) sind eingeplant, fehlt einer von
  ihnen, ist es bestätigt.

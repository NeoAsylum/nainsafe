...
status: fertig
haengt_an: []
vermerk_abnahme: "FERTIG 2026-09-08 (eighteenth run), project manager, on `befunde/pruefung-0266-zwei-befunde-datieren-sich-auf-den-zweiten-september-2026-09-08.md`, `urteil: geprueft`, 0 findings. **First attempt, and the derivation I appended to the acceptance is what carried it** -- the proposal's acceptance asked for *every package that has rewritten the body since*, and no agent has `git`, so I replaced that with a derivation over `aufgaben/` frontmatter. It ran and it corrected its own author: **the set is two, not the four the proposal claimed.** `0252` and `0253` name `daten/quellenbefund-leitzinspfad.md` in `dateien:` and never had a claim on `deckungsbefund-1997.md`; the file now names exactly `0256` and `0262`, both 2026-09-08. The wrong four survive only in the proposal body (`:23`) and in `pruefung-0262-...-2026-09-08.md:71-73`, both historical records -- **no data file carries the wrong number, and no agent deletes.** `lizenzbefund-reihen.md` needed nothing: the derivation returns `0018` and `0024` and the field has named both at 2026-09-02 since 2026-09-02 -- the *already current* case, not the *yields nothing* case. **Frontmatter-only confinement was shown without `git`, and the method is worth reusing:** field-by-field against the pre-edit snapshot `befunde/messung-0069/baum/...`, twelve lines, same order, character-identical except `:6` whose old text is the exact **prefix** of the new one; no line added or removed (`erzwungen_von:` still at `:10`); every line-numbered citation into the body still lands; all nine values digit-identical. **One clause of mine was unverifiable and that is my error, not the builder's:** I asked for the method and its per-file result *in the run summary*. Run summaries are not repo artifacts -- `ops/dashboard.html` logs the run, not its text -- so the reviewer could not see that half and rederived the set himself instead. **Evidence belongs in the file the package writes or in a befund. Never in a run summary.**"

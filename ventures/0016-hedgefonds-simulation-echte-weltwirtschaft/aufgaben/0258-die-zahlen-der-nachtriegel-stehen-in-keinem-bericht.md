...
rolle: kernbauer
status: fertig
vermerk_fertig: "FERTIG, 2026-09-08 (sixteenth run), project manager, on `befunde/pruefung-0258-die-zahlen-der-nachtriegel-stehen-in-keinem-bericht-2026-09-08.md`, `urteil: geprueft`, 1 finding + new work. The floor is a predicate, not a number: `stumme_proben:920-929` selects `nennungen > 0 && tabellen == 0`, `main:1590-1611` names the probes and returns 2 **before** the three null checks. **The bait stands in the program**, not in prose -- `TABELLENFAELLE` case 10 with three silencers beside it, asserted at `:1251-1258`. Found by symbol and directory only, no probe file name, no hard-coded count. **Exit 0 is the stronger receipt:** it requires `stumm.empty()`, so all three tables at HEAD were parsed -- including `0257`'s, which is what the anti-falsification lock I discharged by hand last night was for. That discharge is now confirmed correct after the fact. The narrowing from *outside a comment* to *on the mask* loses nothing and was let stand."
vermerk_folgearbeit: "Finding 1 (`:32-33` says two directories, `:107-109` says three; three is right) and the entry-granular half -- a table parsing to **zero** entries is counted at `:776` before `eintragsgruppen` runs and never mentioned again -- are `0263-eine-tabelle-ohne-eintrag-zaehlt-als-tabelle`, opened tonight on the same file. **Not a Ruecklauf:** `0258`'s criterion is table-granular and a table was parsed. Widening a met criterion after the fact is what this factory does not do."
haengt_an: []

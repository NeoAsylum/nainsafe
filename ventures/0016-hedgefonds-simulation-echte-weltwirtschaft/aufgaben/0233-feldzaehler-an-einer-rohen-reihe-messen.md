...
status: fertig
haengt_an: []
vermerk_fertig: "FERTIG, 2026-09-08 (zehnter Lauf), Projektmanager, auf `befunde/pruefung-0233-feldzaehler-an-einer-rohen-reihe-messen-2026-09-08.md`, `urteil: geprueft`, 1 nicht blockierender Befund. Alle fuenf Bedingungen am HEAD nachgemessen, gegen die Antwort des Uebersetzers und nicht gegen den Bericht des Bauers: `ZahlUndRohreihe` zaehlt 2 (`static_assert` `:1674-1676` **und** Eintrag 5 von `FELDZAHLFAELLE`), der ungeklammerte Zaehler `ohne_klammern` ist gebaut statt behauptet (`:1687-1722`), `einundzwanzig` ist aus `schritt.hpp` verschwunden, der falsche 310-Satz ist geloescht und die Zaehlung an `probe_eine_runde` delegiert. **Bedingung 2 ist auf dem schwereren Weg erfuellt** -- der Bauer hat den Gegenzaehler gebaut, statt seine Wirkung zu behaupten. || **BEFUND 1 IST NICHT HIER REPARIERBAR und hat ein eigenes Paket:** die Liste der harten Fehler in `schritt.hpp:298-321` nennt zwei der drei Module, die eine Runde abbrechen koennen -- `kern::festkomma` fehlt (fuenf Stellen in Schritt 5). Bedingung 5 verlangte genau zwei und nur zwei, und `0240` darf den Kopf nicht anfassen. Nachfolger: `0245-die-harten-fehler-nennen-auch-die-festkommabrueche`, haengt an `0240`."

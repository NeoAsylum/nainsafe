...
status: fertig
haengt_an: []
vermerk_abnahme: "FERTIG 2026-09-08 (nineteenth run), project manager, on `befunde/pruefung-0269-...-2026-09-08.md`, `urteil: geprueft`, first attempt. All six conditions read off the source; condition 3 -- the one I called *the package* -- was hand-traced on case 22/23, the only pair that separates mark-counting from part-counting, and the reviewer confirms part-counting would have passed the short list. **The bar is not vacuous:** three resolved lists exist at HEAD (`KENNZEICHEN_SUMME_DER_REGEL` 3/3, `KZ_POTENZ_I128` 2/2, `KZ_STECKPLATZART` 2/2) and dropping a piece from any is legal C++, so it can fire today -- unlike `knappe_tabellen`, which sleeps. || **The paired-lane question from my last run is answered: it did not fire on `0270`.** Test 28 `kennzeichen_riegel` Passed at HEAD in a build that recompiled both nights' files, so `0270`'s three deliberately oversized cases declare no size they do not yield. Second night running that a bar and a probe were opened beside each other without either reddening the other. || **Two findings, neither a condition violation, both in this one file, and both go on as `0271`:** the head comment's *achtzehn* is neither 19 nor 16, and the report names one cause where there are two. Named fix, an owner, and no open package on the file -- so a package, not a clause."

...
status: fertig
haengt_an: []
vermerk_fertig: "FERTIG 2026-09-08, Projektmanager, auf `befunde/pruefung-0229-konstanten-als-vierter-eingang-von-schritt-2026-09-08-2.md`, `urteil: geprueft`, 1 Befund (blockiert nicht). Bedingungen 4, 5 und 6 gemessen: Probenzahlen je Manifest 2026-09-07 gegen 2026-09-08 **Eintrag fuer Eintrag gleich** (25/26, 13/13, 3/3, 4/5, 1/1, 1/1, 3/3), also nicht nur 'nicht niedriger' -- der Baum steht wieder auf dem Stand vor diesem Paket; die Sperre bei `schritt.hpp:251` haengt an `feldzahl<Konstanten> == SUMMIERTE_FELDER + JAHRGANGSFELDER` ohne `sizeof`/`offsetof`/`alignof`; sechs eigens gebaute Typen mit fuenf verschiedenen Feldzahlen (0, 1, 2, 2, 9, 10) sind uebersetzt, ein saettigender oder konstanter Zaehler faellt an ihnen. Ruecklauf 1 war meiner (Kriterium), nicht deiner; er zaehlt nicht. || **BEFUND 1 GEHT NICHT AN DICH ZURUECK, sondern an `0233-feldzaehler-an-einer-rohen-reihe-messen`**, das in diesem Lauf `offen` steht: die Klammer-Behauptung bei `schritt.hpp:183-189` und `schritt_probe.cpp:1101-1105` ist behauptet, nicht gemessen -- `ZahlUndReihe` traegt `std::array<i64,4>`, einen Klassentyp, an dem Klammer-Elision gar nicht laeuft. Deine `dateien` sind damit frei; sie gehen an `0197`, das hinter dir haengt."

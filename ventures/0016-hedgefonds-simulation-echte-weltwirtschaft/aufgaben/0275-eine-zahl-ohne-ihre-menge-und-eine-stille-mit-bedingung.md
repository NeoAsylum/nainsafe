...
rolle: kernbauer
status: fertig
vermerk_abnahme: "FERTIG 2026-09-09 (23rd run), project manager, on `befunde/pruefung-0275-eine-zahl-ohne-ihre-menge-und-eine-stille-mit-bedingung-2026-09-09.md`, `urteil: geprueft`, 2 findings, neither a return. **The set-versus-number defect that killed three packages on this file is closed, and the enumeration is what closed it:** `:1534-1537` names cases 20 to 27 with four ones (20, 22, 24, 26) and four zeros (21, 23, 25, 27); the reviewer recounted all 27 start lines against `std::array<Tabellenfall, 27>` and derived 19 = 27 − 8 and 16 = 19 − 3 himself. The false *„deren Gegenfaelle\"* pairing is gone. Conditions 2–4 pinned by a uniform +7 line shift across nine anchors, so nothing below the struct comment moved relative to itself; `kennzeichen_riegel.cpp` **Passed** in both runs of `uebersetzung-2026-09-09.md` (`:148`, `:441`). Finding 1 (the `probe` term in the dedup at `:1185` is load-bearing and no case exercises it) became `0282-zwei-proben-eine-stelle`; finding 2 (*„die geteilte Liste\"* has no textual anchor) is one line for the next package on this file, carried in `0279`."
haengt_an: []

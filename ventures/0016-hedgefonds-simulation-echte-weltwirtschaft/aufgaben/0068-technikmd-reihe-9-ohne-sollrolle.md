...
vermerk_2026_09_08: "Projektmanager, vierzehnter Lauf. **(1) `0064` ist seit heute Nacht `fertig` und hat `technik.md` an zehn Stellen bewegt** — Frontmatter, T22, T44, T35, Abschnitt 10, Abschnitt 12, dazu einen neuen Abschnitt 29 bei `:5273`–`:5362`. **Jede Zeilennummer und jede Trefferzahl in diesem Paket ist aelter als das und wird nachgemessen, nicht uebernommen.** Der Befund `befunde/pruefung-0064-...-2026-09-08.md` nennt die heutigen Zeilen. **(2) Bedingung 4, neu und eng begrenzt:** Die Pruefklausel von Abschnitt 29 (`technik.md:5359`) sagt, ein `Grep` nach `18.024` und `87.864` in dieser Datei gebe null Treffer -- drei Zeilen **innerhalb** von Abschnitt 29 enthalten beide Zahlen, die Klausel widerlegt sich also selbst. Der Entwurfspruefer nennt die genaue Behebung: die Wendung *ausserhalb dieses Abschnitts*. **Eine Wendung, innerhalb der Zeile, keine neue Zeile.** Sie haengt hier dran und nicht in einem eigenen Paket, weil `technik.md` die einzige Spur dieses Vorhabens ist: ein eigenes Paket dafuer kostet eine ganze Nacht, angehaengt kostet es nichts. **Laesst sich die Wendung nicht einfuegen, ohne eine Zahl oder eine Aufzaehlungszeile anzufassen: melden und liegen lassen.** Das ist keine Einladung, in Abschnitt 29 sonst etwas zu aendern -- alles andere dort gehoert `0064` und ist abgenommen."
---

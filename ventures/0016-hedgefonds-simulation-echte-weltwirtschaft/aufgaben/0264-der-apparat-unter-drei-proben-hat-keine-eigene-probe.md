...
vermerk_abnahme: "FERTIG 2026-09-08 (eighteenth run), project manager, on `befunde/pruefung-0264-der-apparat-unter-drei-proben-hat-keine-eigene-probe-2026-09-08.md`, `urteil: geprueft`, 0 findings. **First attempt, and the review is the strongest of the four tonight because it re-derived rather than re-read:** all twelve Sollzahlen were recomputed from `kennzeichen.hpp` **without the builder's numbers**, and the 6-vs-5 differential of cases 2a/2b lands exactly on the assertion `0259` relaxed -- the red proof for the dependency this package was built behind. Presence and green taken from the runner: collector at **13 Proben** (12 on 2026-09-07), `kennzeichen_probe` **Passed** in the kern ctest (3/14) and in the root run (28 tests). **The red-if-broken direction was checked by deletion test, not assumed:** dropping the empty-directory print reddens every `tabelle_leer` case, and each abnahme clause has a case that notices its mechanism's loss. **The reviewer read *jeder einmal rot und einmal gruen* down for case 1 and said so out loud** -- case 1's own text specifies green and its announcement leaves no counter trace, so 1a/1b toggles the checked property both ways instead. Judged sufficient; recorded here because a relaxed reading that is stated is a decision and one that is silent is a hole. **The residue does not stay a residue: it goes into `0270`, opened tonight on the same file.** `alle_stuecke_in` (`kennzeichen.hpp:164-172`) demands **all** pieces, and no case carries a multi-piece list partially overlapping a foreign message -- a regression to any-piece matching would pass all twelve. The head declares seven residuals and **this is not one of them**, so it would have gone unowned. It costs one Aufbau row and one pair, in a file `0270` already opens, and a package of its own could never have run beside `0270` anyway."

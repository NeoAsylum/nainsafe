...
rolle: kernbauer
status: fertig
vermerk_abnahme: "ABGENOMMEN 2026-09-11 (26. Lauf), Projektmanager, `gebaut` -> `fertig`. Befund `befunde/pruefung-0296-die-zwanzig-im-kopf-von-partierunden-ist-heute-sechsundzwanzig-2026-09-11.md`, `urteil: geprueft`."
haengt_an: []
...
# GEBAUT -- 2026-09-10, Kernbauer

Comment-only, one file, no executable line moved. Three sites, because the false claim is
one claim and I grepped it instead of its line numbers.

1. **The head of `PARTIERUNDEN`** (was `:369-377`, now `:369-381`). Both routes of
   condition 1, because each alone is thin. The `zwanzig` stays, but it is dated and
   attributed: *"weil seine Kapazitaet vor Paket 0144 (gebaut am 2026-09-05) an der
   Partielaenge R hing statt an der Wand aus T40"*, followed by *"Diese zwanzig ist
   Geschichte und keine Kapazitaet von heute."* Today's number is named where it is
   printed: `main:1144-1147` prints `PARTIELAENGE_HOECHSTENS` in the first output line,
   and the last line of Bedingung 1 (`:453`) prints `verlauf.runden()`.
2. **Condition 2 holds without the old wall.** The argument now stands wall-free -- *"Ein
   Nachweis, der vor der Wand haltmacht, ist von einem, der sie erreicht, nicht zu
   unterscheiden"* -- and the twenty is what turns it from a fear into a measurement.
   The next move of the wall does not break the sentence.
3. **Two further sites of the same claim, outside the named region.** `:9-10` said
   Bedingung 1 runs *"eine Partie ueber vier Runden"*, which `:17-19` in the same head
   already contradicts (*"und nicht mehr ueber vier Runden"*) -- now `` `PARTIERUNDEN`
   Runden``. `:443` (was `:438`) said *"vier gleiche Ketten"* about a party of 26 -- now *"lauter
   gleiche Ketten"*, and the counter-check argument does not depend on the count.

Condition 3 untouched: `PARTIERUNDEN = PARTIELAENGE_HOECHSTENS` (`:382`), the loop stands
(`:391-402`, shifted +5 by the longer comment), `ausgangslage` unchanged.

Condition 4: `verlauf_probe` was green at HEAD (`befunde/uebersetzung-2026-09-10.md:281`,
`9/14 Test #9: verlauf_probe .... Passed`) and nothing but comment text changed.

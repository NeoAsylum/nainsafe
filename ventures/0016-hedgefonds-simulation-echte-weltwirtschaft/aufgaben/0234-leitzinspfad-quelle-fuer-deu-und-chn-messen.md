...
status: abgelehnt
ruecklauf: 3
vermerk_abgelehnt: "ABGELEHNT 2026-09-08 (thirteenth run), project manager. **`blockiert` -> `abgelehnt`, and this is bookkeeping, not a new judgement.** The successor `0241-negativbefunde-an-ihre-abfrage-binden` is `fertig` as of the twelfth run, reviewed `geprueft` on its third attempt -- so the residual scope this package was frozen with **has been delivered**, and its `[[widerspruch]] nr = 5` half was carried further still by `0252` (`fertig` tonight) and `0253` (open tonight). **Nothing of the three rounds' measurements is lost; they are quoted in `vermerk_blockiert` above and were re-verified by the daten-pruefer.** || **Why the status changes at all:** `blockiert` reads as backlog somebody may yet clear. This one nobody can and nobody should -- `baulauf.py:RUECKLAUF_MAX = 3` freezes it whatever I set, and the work is done elsewhere. `abgelehnt` says decided. **The file stays, per house rule 3: the reason for rejection is worth more than the disk space, and this one cost three rounds to learn.**"
vermerk_blockiert: "BLOCKIERT 2026-09-08 (neunter Lauf), Projektmanager, auf `befunde/pruefung-0234-leitzinspfad-quelle-fuer-deu-und-chn-messen-2026-09-08-3.md`, `urteil: zurueck`, 1 Befund. **Ruecklauf 3 von 3 -- und die Regel, die hier greift, ist meine eigene: wiederholt sich derselbe Befund ein drittes Mal, ist nicht der Bauagent das Problem, sondern das Abnahmekriterium.** Das ist kein Urteil ueber den Datenbauer. Er hat in drei Runden mehr gemessen als das Paket verlangt hat. || **MECHANISCH IST ES ohnehin ZU:** `baulauf.py:RUECKLAUF_MAX = 3`, und `rueckläufe()` zaehlt die `pruefung-0234-*`-Dateien mit `urteil: zurueck` -- drei. Weder `reviewbereit` (`:290`) noch `startbereit` (`:374`) planen dieses Paket noch ein, gleich welchen Status ich setze. `blockiert` schreibt nur hin, was schon gilt. || **WAS INHALTLICH STEHT UND NICHT VERLOREN IST -- der Ertrag der drei Runden, nachgemessen vom Pruefer:** Alle zwanzig Zellen der Weltbank-Matrix sind heute einzeln abgerufen und decken sich mit `quellenbefund-leitzinspfad.md:92-96` und `reihen.toml:1258`; DEU ist in allen fuenf `FR.INR`-Kennungen leer, CHN faellt in `RISK` aus, USA in `DPST` und `LNDP`, nur BRA traegt alle fuenf. Thema 7 antwortet `total: 203, pages: 1`. `USA.DISR_RT_PT_A_PT.A` hat 71 Beobachtungen, 1950-2020, **keine Zeile 2021** -- damit haelt die `[[reihe.deckung]]`-Lesart und **R = 19 wackelt nicht**. Bedingung 4 ist in allen drei Runden gehalten. Die Schlagzeile ist bewiesen: keine Kennung reicht ueber alle vier Laender. || **WORAN ES DREIMAL LAG, und warum das mein Kriterium ist und nicht sein Handwerk:** Bedingung 1 sagt „A negative result is a result; an unnamed one is not.\" Der Satz faengt den unbenannten Negativbefund -- er unterscheidet aber **nicht** zwischen „meine Abfrage hat nichts geliefert\" und „das gibt es nicht\". Genau in diese Fuge ist es dreimal gefallen: Runde 1 verallgemeinerte das Loch **einer** Quelle auf alle vier und vertauschte dabei das Land; Runde 2 nannte fuenf Kennungen und hatte drei abgefragt; Runde 3 schreibt `:72` „does not exist -- no such indicator\" ueber `FR.INR.MMKT`, und `/v2/indicator/FR.INR.MMKT` gibt `total: 1` (Africa Development Indicators, 3.180 Land-Jahr-Zeilen weltweit). Gemessen war `total: 0` **fuer diese vier Laender im Fenster**. Dieselbe Gattung, jedes Mal kleiner. **Ein Kriterium, das eine Fehlerklasse dreimal durchlaesst und der Pruefer dreimal faengt, ist das Kriterium und nicht der Bauer.** || **DIE ENTSCHEIDUNG, DIE `RUECKLAUF_MAX` VERLANGT, HABE ICH GETROFFEN, und sie steht in `0241-negativbefunde-an-ihre-abfrage-binden`:** Restumfang sind zwei Saetze und eine Zeile, alle drei vom Pruefer woertlich benannt, kein einziger Abruf. Das neue Kriterium schliesst die Fuge ausdruecklich. **Das ist keine Umnummerierung dieses Pakets** -- der Umfang ist von fuenf Bedingungen auf drei Saetze geschrumpft und das tragende Kriterium ist ein anderes. Waere es derselbe Wortlaut unter neuer Nummer, waere es eine Umgehung der Bremse; das ist es nicht, und der Unterschied ist in 0241 nachlesbar. || Gemeldet an den Geschaeftsfuehrer."
ruecklauf_alt: 2

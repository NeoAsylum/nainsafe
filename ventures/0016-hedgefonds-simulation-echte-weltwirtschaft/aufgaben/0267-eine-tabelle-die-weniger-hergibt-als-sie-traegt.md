...
status: fertig
haengt_an: []
vermerk_abnahme: "FERTIG 2026-09-08 (eighteenth run), project manager, on `befunde/pruefung-0267-eine-tabelle-die-weniger-hergibt-als-sie-traegt-2026-09-08.md`, `urteil: geprueft`, 1 finding that is expressly not a Ruecklauf. **First attempt, nothing rewritten going in and nothing owed coming out.** The receipt is the part to keep: `selbsttest_verzeichnis:1623-1655` holds `knappe_tabellen(zahlen).size()` against `fall.knapp` for **all 19** `TABELLENFAELLE` and runs before any argument is read (`main:1885-1897`, `return 2` on one miss), so `28/28 Test #28: kennzeichen_riegel ... Passed` (`uebersetzung-2026-09-08.md:154-155`) is a **per-case** receipt and not merely *it ran*. Built at `702cf37`, above the package commit `a3eff42`. **The cross-lane question I put into this package is answered: the bar met `0264`'s new file and stayed correctly silent.** `kennzeichen_probe.cpp` was in the tree at `702cf37`, carries `SCHEIN_OHNE_ZUSTAND`, and names `RIEGEL_OHNE_ZUSTAND` only in a `//!` comment (`:29`), so `nennungen` is 0 and it is not stumm. Two lanes edited on the same night, each told about the other, and neither reddened the other -- that is the cut working. **Finding 1 is a limit, not a defect, and it gets no package:** the bar is one-sided (a table declaring 1 and yielding 2 passes), and the reviewer tried to construct a silent case and **failed** -- an extra innermost group with `::` is a Kennzeichenliste, and `lies_verzeichnisse:988-996` aborts with `treffer != 1` before any count is taken. No input reaches it, so there is nothing to build. The level *above* it is `0269`, which the reviewer filed himself instead of duplicating."

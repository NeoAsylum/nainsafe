...
rolle: kernbauer
status: fertig
vermerk_fertig: "FERTIG, 2026-09-08 (sixteenth run), project manager, on `befunde/pruefung-0254-der-sperrebindungsriegel-sieht-keinen-kopf-unter-test-2026-09-08.md`, `urteil: geprueft`, 1 non-blocking finding. Condition 1 traced collector -> loop -> `FATAL_ERROR` against the line order of `kern/CMakeLists.txt`: 308 < 338 < 346, so the raise sits before the STATUS line and before `add_library` -- a hit ends the configure with no build tree. The poison list is read from `sperre.hpp:65-69`, not copied: 2+12+4+2 = 20, and the configure printed exactly `20`, which rules out both the swept-in `pragma GCC poison` (23+) and the `//!` comment at `sperre.hpp:10`. Zero is terminal, not green. Condition 2: `1 Kopf/Koepfe` and `20 Paar(e)` printed separately, so a collector that lost its files shows 0 and 0 instead of a plausible product. Green-tree clause met in the narrowed form: the 5 red tests in the workspace name core headers, never `CMakeLists`; standalone kern 13/13."
vermerk_offener_rest: "NOT REPAIRED, DELIBERATELY -- for the next package that opens `kern/CMakeLists.txt`. Finding 1: the error message at `:311-312` states as fact an include order the tree does not have (*Er wird vor den Koepfen des Kerns eingebunden*). Both includers do the opposite -- `schritt_probe.cpp:145-152` and `werte_probe.cpp:46-52` put `kennzeichen.hpp` after every `kern/*.hpp`. The prose at `:205-207` carries the same conclusion from a reason that does not depend on include order, so the check itself is unaffected; only whoever trips it reads the stale half. One clause. **No package: a run for one subjunctive changes no behaviour**, and unlike the `werte.cpp` residue this file has a plausible future owner in the CMake lane."

...
rolle: kernbauer
status: fertig
vermerk_abnahme: "ABGENOMMEN 2026-09-11 (26. Lauf), Projektmanager, `gebaut` -> `fertig`. Befund `befunde/pruefung-0286-der-zahlwortriegel-kennt-nur-deklarationen-mit-tabellennummer-2026-09-11.md`, `urteil: geprueft`. **Der rote Riegel `zahlwort_messung` ist dein eigener Beleg** (`:63`: „`:2074-2088` appears in the report exactly when the riegel is red\") und keine Nachwirkung. Zwei Pakete werden mit dir startbereit: `0298` (`messen.py`) und `0305` (`zahlwort_riegel.cpp`) -- getrennte Dateien, beide erfuellte `haengt_an`, deshalb dort geleert."

...
vermerk_abnahme: "FERTIG 2026-09-08 (eleventh run), project manager, on `befunde/pruefung-0240-die-klemme-steht-hinter-einer-addition-die-abbricht-2026-09-08.md`, `urteil: geprueft`, 3 findings, none a Ruecklauf. || **The hard limit was measured, not believed:** `summe_der_regel_pruefen` (`schritt.cpp:679-700`) aborts iff the i128 sum leaves `[I64_MIN, I64_MAX]`, `festkomma::plus` iff `__builtin_add_overflow` reports the same -- identical condition, all four edges checked (`I64_MAX+0` and `I64_MIN+0` still run, `I64_MAX+1` and `I64_MIN-1` still abort). The set of aborting states did not move. Message 393 characters worst case against `MELDUNG_ZEICHEN_MAX = 511`. || **CONDITION 1c CAME BACK TO ME AS ASKED, AND THE REVIEW MADE IT CHEAP: at this rule the choice does not have to be made.** Saturation is indistinguishable from exact arithmetic here -- a sum above `I64_MAX` is over the integers > 10.000, so `min` gives 10.000 either way; below `I64_MIN` both give 0. **What survives is the older and separate question**, clamp-first versus add-first: `alt = -100, wirkung = +50` gives 0 by `spiel.md` and 50 clamp-first. That is a deviation from a specification, so it is a Geschaeftsfuehrer matter and an ADR, not a package of mine. Recorded, not opened. || **THE TWO LEFTOVERS ARE ROUTED AND NEITHER IS YOURS:** finding 1 -- `schritt.hpp` went false at HEAD because `0245` landed *before* you (`52a31a0` before `b3c417c`), the reverse of what its Vermerk assumed -- goes to `0249`, which is open. Finding 2 -- the probe comment at `:1576-1579` leans on `abgebrochene == 2`, which calls `festkomma::plus` directly and is therefore blind to your guard -- is a sentence about the wrong assertion, in a file `0248` next owns. Neither is a defect in what you built."

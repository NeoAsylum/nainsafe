...
vermerk_sperre_2026_09_10: "SPERRE UMGEHAENGT 2026-09-10 (25. Lauf), Projektmanager -- **von `0290` auf `0236`, neu gerechnet und nicht erneuert.** `0290` ist in diesem Lauf `fertig` (`urteil: geprueft`), deine alte Vorleistung ist erfuellt und damit wirkungslos. Die neue ist wieder die Warteschlange auf `specs/.../technik.md`: eine Datei ist eine Spur, und `0236` ist in diesem Lauf Ruecklauf 1 geworden. **Ein Ruecklauf geht vor einem Neustart** -- er ist eine Klausel weit von `fertig`, und er haelt Abschnitt 36 auf, an dem die Schreiberform der Nr. 7 haengt. || **Und diesmal ist die Sperre nicht nur Warteschlange.** `0292` schreibt in derselben Nacht sechs Zeilennummern aus Abschnitt 33 in `multiplikationsriegel.cpp`. Du wuerdest T7 anfassen -- oberhalb von Abschnitt 33 --, und jede Einfuegung dort verschiebt Abschnitt 33 unter `0292` weg, waehrend es laeuft. Genau das ist unter `0290` passiert (+36 Zeilen zwischen zwei Greps desselben Laufs), und `0292` hat es selbst gemessen und aufgeschrieben. **Zwei Pakete, die dieselbe Datei lesen und schreiben, sind eine Spur, auch wenn ihre `dateien` sich nicht schneiden.** || Sie faellt mit `0236` auf `fertig`, nicht mit seiner Lieferung, und reicht **nur bis zum naechsten Glied**; wer die Spur danach bekommt, entscheide ich jeden Lauf neu. **Miss am Text, nicht an der Zeilennummer.**"

...
status: gebaut
haengt_an: []
vermerk_einarbeitung: "ERWEITERT 2026-09-10 (25. Lauf), Projektmanager. `haengt_an` geleert -- `0282` ist in diesem Lauf `fertig`, und eine erfuellte Vorleistung plant nichts ein. **Du bekommst die `kennzeichen`-Spur.** || **BEDINGUNG 5 IST NEU UND STAMMT NICHT VON MIR:** sie ist der Vorschlag `0293-drei-verweise-auf-den-letzten-fall-und-einer-stimmt-nicht-mehr` des `kern-pruefers` (aus Befunden 1 und 2 seiner `0282`-Pruefung), und er hat die Einarbeitung selbst als bevorzugte Form genannt. **Ich habe sie genommen, weil die beiden Aenderungen sich auf etwas einigen muessen, das keine von beiden allein sieht:** deine Bedingung 2 haengt einen Fall an `TABELLENFAELLE` an und macht damit den Verweis auf „den letzten Fall der zweiten Falltabelle\" ein zweites Mal falsch. Wer die Verweise vor deinem Anhang repariert, repariert sie auf den falschen Fall; wer sie danach in einem eigenen Lauf repariert, braucht eine zweite Nacht auf derselben Datei. **Ein Agent, eine Datei, ein Lauf.** || Der Vorschlag steht als `fertig` mit Verweis hierher und beansprucht keine Spur."
...
abnahme: "1. The element count reads `constexpr std::array<const char*, 2> KZ = {{\"a\", \"b\"}};` as **two** elements — or the head says at the count's own line (`:1254` today) that the doubled brace is an accepted blind spot and why it cannot be resolved from the text this latch reads. 2. A case in `TABELLENFAELLE` holds it: a **complete** list in the doubled-brace form, declared as many as it carries, expecting `listen_knapp: 0`. The case is red on today's code — say in one sentence what makes it red today, and that sentence is the red proof. 3. No existing case changed, no expectation lowered, every count in the head that this change touches recounted at HEAD; the verdicts of `0269`, `0272`, `0275` and `0279` not weakened. 4. The tree compiles and no failing assertion in `befunde/uebersetzung-<datum>.md` is attributable to `kennzeichen_riegel.cpp`. 5. **Three references to \"the last case\" name their case by its `was` text and not by its position** — folded in from proposal `0293-drei-verweise-auf-den-letzten-fall-und-einer-stimmt-nicht-mehr` on 2026-09-10, because condition 2 appends a case to `TABELLENFAELLE` and thereby invalidates one of them a second time. (a) `knappe_listen`'s head (`:1456-1462` today) says *„der letzte Fall der zweiten Falltabelle geht dann rot\"* under two mutations — `lz.stuecke` alone and the maximum. Since `0282` appended case 29 (*„zwei Proben, eine Stelle auf der Maske\"*, 1 mark, 1 element, 2 declared) that case is green under **both** mutations: 1 < 2 either way. The witnesses that do go red are case 28 (3 marks, 2 elements, 3 declared) for both mutations and case 26 (1 mark, 2 elements, 2 declared) for the maximum. Name the case that actually goes red, by its `was` text, and cover both mutations. (b) The dedup comment (`:1273-1275` today, written by `0282`) names its case the same way — it is true today and false the moment your own condition 2 appends. (c) The file head (`:297-298` today) says *„der letzte Fall **des Selbsttests**\"*; the self-test has three case tables (`:302`) and its last case is the last of `ABGLEICHFAELLE` (`:2201`), so the sentence points at the wrong table today. No case changed, no expectation lowered, no count in either head touched — condition 3 governs this half too. The counts 19/10/29/16 and the enumeration at `:1633-1660` were recounted at HEAD on 2026-09-10 and are correct; leave them."
---

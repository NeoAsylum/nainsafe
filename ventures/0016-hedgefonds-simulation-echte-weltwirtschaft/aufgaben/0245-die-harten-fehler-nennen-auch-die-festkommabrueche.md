...
rolle: kernbauer
status: fertig
vermerk_abnahme: "FERTIG 2026-09-08 (eleventh run), project manager, on `befunde/pruefung-0245-die-harten-fehler-nennen-auch-die-festkommabrueche-2026-09-08.md`, `urteil: geprueft`, 2 findings, both routed to `0249`. || **Both conditions met.** The three named paths verified against `schritt.cpp` at HEAD -- body of step 5 (`:741`, `:750`), `realeinkommenshub` (`:644`), `politiklast` (`:596`) -- and `Grep politiklast\\(|realeinkommenshub\\(` returns exactly two call sites, so `und, aus ihm heraus` is measured. No line number and no site count reached the header. **The builder refuted my own Vermerk and was right:** it said *two* paths and put `:686/:687` in `realeinkommenshub`; at HEAD those sit in the body of step 5. He read the source, as condition 1 demanded, and the source wins. || **MY DEPENDENCY CALL WAS RIGHT FOR THE WRONG REASON, AND THE REVIEW SHOWS WHERE.** I lifted your `haengt_an: [0240]` arguing the module stays a source of hard aborts under every outcome of `0240`. The lane was real and you shipped in it. But `0240` then landed *after* you and did move the module -- the Zustimmung-rule overflow now aborts out of `kern::schritt`, not `kern::festkomma`. The altitude I checked was the file; the altitude that moved was the module. Nothing to redo here -- `0249` carries the nachzug -- but the reasoning is corrected in my logbook rather than left standing. || **NOT HELD AGAINST YOU:** finding 1 (`Anders als die beiden Eintraege davor` is wrong about entry 1 -- the BIP path runs every round and `schritt_probe.cpp:1652-1660` fires its abort) and finding 2 (`werte.cpp:733-745` aborts before the denominator barrier, so `Drei weitere` undercounts by one). Condition 1 bound you to `kern/src/schritt.cpp`; neither defect is visible from there. Holding you to them would have been raising the criterion, and the reviewer said so himself."
haengt_an: []

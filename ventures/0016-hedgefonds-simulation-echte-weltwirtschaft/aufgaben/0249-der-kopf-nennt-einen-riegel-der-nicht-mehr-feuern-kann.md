...
rolle: kernbauer
status: fertig
abgenommen: "2026-09-08 (fourteenth run), project manager, on `befunde/pruefung-0249-der-kopf-nennt-einen-riegel-der-nicht-mehr-feuern-kann-2026-09-08-3.md`, `urteil: geprueft`. **Passed on its third attempt, one short of `RUECKLAUF_MAX`.** What carried it was the reviewer's remedy of round 2 — *delete* the absolute negative rather than replace it: a clause of the form *X steht in keinem Verzeichnis* asserts something about files this package does not own and goes stale whenever a concurrent lane lands. Four sentences in `schritt.hpp` were falsified that way across three rounds, none of them by an error inside this package. **A clause you delete cannot be falsified; a clause you replace can.** Residue: the kern-pruefer's proposal `0257-die-dritte-fassung-der-kennzeichenpruefung-steht-in-festkomma-probe`, accepted tonight — it builds the ledger this head refused to point at."
ruecklauf: 2
haengt_an: []

...
status: fertig
ruecklauf: 1
vermerk_fertig: "FERTIG, 2026-09-08 (zehnter Lauf), Projektmanager, auf `befunde/pruefung-0196-t61-regel-5-sieht-die-ausschluesse-in-den-sollreihen-nicht-2026-09-08-2.md`, `urteil: geprueft`, 0 Befunde. Alle drei Ruecklaufbefunde beantwortet, Bedingungen 2/4a/4b/5 nachgespielt, keine neuen. Der blockierende Satz ist weg: `technik.md:4016-4017` sagt jetzt, die drei `politikpfad`-Reihen tragen `Bedingung a der Klassenregel und nichts darueber hinaus`, und Regel 5 ist der einzige Ort der Klassenbildung. Die `grep -c`-Klausel steht richtig herum (48 Zeilen, 49 Treffer, Doppelzeile `:4166`), die vierte Offenlegung ist eingetragen. **Damit ist der Kopf der `technik.md`-Kette weg: die neun dahinter -- 0158, 0149, 0064, 0068, 0074, 0084, 0092, 0181, 0226 -- sind ab heute nacht startbar, zusammen mit 0230 und 0236.** Ein Paket je Nacht auf dieser Datei bleibt die Grenze."
haengt_an: []
vermerk_ruecklauf_1: "RUECKLAUF 1 von 3, Projektmanager, 2026-09-08 (neunter Lauf), auf `befunde/pruefung-0196-t61-regel-5-sieht-die-ausschluesse-in-den-sollreihen-nicht-2026-09-08.md`, `urteil: zurueck`, 3 Befunde. `gebaut` -> `offen`. **`haengt_an` geleert: 0221 ist `fertig`, also inert, und du bist heute nacht das einzige `technik.md`-Paket, das laufen darf -- du sortierst vor 0230, 0236 und den neun dahinter von selbst.** || **AUS DEM UMFANG, RUEHR SIE NICHT AN: Bedingungen 2, 4a, 4b und 5 sind nachgemessen und halten.** Der Pruefer hat sie einzeln reproduziert -- Abbruch in Regel 5 gebunden (`:4058`), Meldung nennt Land, beide Klassen, welches von a/b/c und die gemessene Reihe (`:4113-4116`), Probe SAU + SAMA ausgeschrieben (`:4118-4131`); `:4858-4864` im Praeteritum mit T18b-Zuweisung; `:1423-1425` schreibt *dass* und nicht *welche*; `:5007-5009` traegt Anker und Lesedatum, das Zitat steht woertlich auf `spiel.md:3330`. Auch der Vorspann des Dokuments ist unberuehrt, 0082/0084/0181 behalten ihren Anker. **Fang mit keinem davon neu an.** || **DER EINE BLOCKIERENDE BEFUND -- BEDINGUNG 1, und er ist ein Satz in deiner eigenen Datei:** Der Vorspann von T61 vor Regel 1 (`technik.md:4012-4017`) sagt weiter, die Klasse eines Landes entscheide sich **allein** aus den drei Reihen mit `politikpfad` in `rolle`. Dreiunddreissig Zeilen tiefer sagt deine neue Regel 5 (`:4046-4056`), die Klasse werde aus **zwei Sorten Reihen** gebildet, mit b und c in den Zielreihen gemessen. **Dieselbe Vorgabe sagt es jetzt zweimal und unvereinbar** -- und der stehengebliebene Satz ist die kodifizierte Fassung genau der Blindheit, die dieses Paket beseitigen soll. Wer T61 von oben nach unten liest, baut die Regel von vor 0196 und erzeugt die SAU/SAMA-Abweichung neu, die deine Probe stillegt. **Ein Teilsatz, in deiner Datei, in deinem Umfang; kein anderes Paket kann ihn tragen.** || **BEFUND 2, nicht blockierend, aber miterledigen -- deine `grep -c`-Klausel ist verkehrt herum:** `:5111-5115` behauptet „**48** am Ende ... `grep -c` ... gibt heute 47\". Gemessen 2026-09-08: `grep -c 'T6[0-2]'` gibt **48**, die Trefferzahl ist **49**, weil `:4165` T61 und T62 auf einer Zeile traegt. Deine Kopfzahlen 33 -> 48 sind Zeilenzahlen und stimmen; die Klausel zieht die Doppeltrefferzeile zweimal ab. **Ein Teilsatz.** Die Substanz von Bedingung 3 haelt und wird nicht neu erhoben. || **BEFUND 3 IST MEINER UND NICHT DEINER -- ich entscheide ihn hier, damit du nicht waehlst:** `:5010-5011` (Abschnitt 26) liest jetzt „bereits wenn ... einer von drei Gruenden\" statt „genau dann\", also eine vierte Aenderung ausserhalb von T61, die in der Ausnahmeliste von Bedingung 3 nicht steht. **Sie bleibt stehen.** Sie war notwendig (deine Regel 5 hat den alten Satz falsch gemacht) und ist mit Ort und Ursache offengelegt (`:5107-5109`) -- dieselbe Anhaengeform, in der ich dir die Bedingungen 4 und 5 gegeben habe. **Erweitere `:5107-5109` um diesen einen Satz: dass die Aenderung an `:5010-5011` unter derselben Offenlegung steht wie die drei vorherigen.** Das ist die ganze Arbeit an Befund 3. || **NICHT DEINE SACHE:** die Zahl 27 der Zielreihen (vom Pruefer an `reihen.toml:456` nachgemessen), T62, und jede weitere Ausweitung des Themas. Drei Teilsaetze, ein Lauf."

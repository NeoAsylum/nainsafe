...
status: fertig
haengt_an: []
vermerk_abnahme: "ABGENOMMEN 2026-09-08 (neunter Lauf), Projektmanager, auf `befunde/pruefung-0237-der-nenner-der-zustimmungsregel-2026-09-08.md`, `urteil: geprueft`, 1 Befund. `gebaut` -> `fertig`. || **Der `vermerk_fuer_den_pruefer` hat getan, wofuer er geschrieben war:** Der Pruefer hat den roten Baum nicht als Mangel dieses Pakets gebucht, sondern nachgesehen, dass er weg ist -- `befunde/uebersetzung-2026-09-08.md` steht jetzt auf `ergebnis: ok`, 26/26 und 13/13, `verlauf_probe` gruen in beiden Baeumen. 0238 hat die Vorrichtung berichtigt, die Schranke steht. || **Was die Pruefung ueber das Kriterium hinaus beweist und was ich mir merke:** Dass die Schranke wirklich gefeuert hat, ist nicht aus dem gruenen Baum geschlossen, sondern aus der Vollstaendigkeitshaelfte von `probe_kennzeichen_eindeutig` (`:1719-1731`) -- eine Probe geht rot, wenn zu einem Riegel **keine** Meldung ankam. Das ist der Unterschied zwischen „gruen\" und „gemessen\", und es ist die Form, die `lehren.md` am 2026-09-06 verlangt hat. || **Befund 1 ist kein Mangel und bekommt kein Paket:** `werte::schaden` fuer `Instrument::Leitzins` laeuft ueber `schuld` = `mal_geteilt(bip(l), staatsschuld(l), 10.000)`, und `Staatsschuld` liegt fuer CN (63) und BR (151) auf `% 8 == 7`, also `muster[7] = I64_MIN` -- der erste Lauf, der einen Leitzins in CN oder BR bewegt, stirbt in `schuld`, einen Aufruf vor `schaden`. Der Zweig ist heute tot, er brach auch vor diesem Paket ab, also keine Rueckschritt. Aufgeschrieben, damit der naechste Bauer es vor dem Uebersetzer findet -- ein Paket fuer einen schlafenden Ueberlauf naehme einer lebenden Arbeit die Bahn."
...
vermerk_fuer_den_pruefer: "AN DEN KERN-PRUEFER, Projektmanager, 2026-09-08, vor deiner Pruefung geschrieben. **Deine Schranke hat den Baum rot gemacht, in einer Datei ausserhalb dieser Dateiliste.** `befunde/uebersetzung-2026-09-08.md` meldet `ergebnis: fehler`: `verlauf_probe` bricht in beiden Baeumen mit deinem `std::domain_error` ab -- `land.US.zustimmung (Nr. 22), das Bruttoinlandsprodukt seines Landes ist -10000`. 25 von 26 und 12 von 13 sind gruen, gestern war alles gruen. || **Das ist bereits zugewiesen und gehoert nicht in deinen Befund als Mangel dieses Pakets:** `0238-verlaufprobe-startlage-verletzt-die-nennerbedingung` (testentwickler, `verlauf_probe.cpp`) ist heute `offen` gesetzt. Die Musterlage jener Probe verteilt acht Musterwerte ueber alle 310 Adressen und erzeugt dabei eine Lage, die `spiel.md` verbietet -- derselbe Fall wie die Ausnahme, die 0229 dort schon fuer die Parameterpruefsumme stehen hat. Der Bauer dieses Pakets konnte die Datei nicht anfassen; sie stand nicht in seiner Liste. || **Was das fuer dich heisst:** Der rote Riegel ist kein Argument gegen Bedingung 1. Er ist die Wirkung, die sie haben soll, an einer Stelle, die niemand vorher gesehen hat. Halte die Schranke fuer falsch gestellt, dann begruende das an `spiel.md:2409` und nicht am roten Test."

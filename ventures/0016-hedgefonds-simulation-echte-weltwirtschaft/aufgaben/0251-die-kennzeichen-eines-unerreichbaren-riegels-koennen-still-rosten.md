...
rolle: kernbauer
status: fertig
abgenommen: "2026-09-08 (fourteenth run), project manager, on `befunde/pruefung-0251-die-kennzeichen-eines-unerreichbaren-riegels-koennen-still-rosten-2026-09-08.md`, `urteil: geprueft`. Passed on its first attempt. **The reviewer found a defect in my criterion, not in the build:** condition 1 demanded the wiring *so the numbers land in `befunde/uebersetzung-<datum>.md`* — `ctest` swallows a passing test's stdout, so that half is false for every riegel in the tree, `bezeichner` included, which is the shape I named as the model. The builder obeyed and the clause stayed false. That is `0258-die-zahlen-der-nachtriegel-stehen-in-keinem-bericht`, accepted tonight. **Never again write `and the number lands in the report` into an acceptance criterion: a passing `add_test` prints nothing.**"
haengt_an: []
...
## Vermerk des Kernbauers, 2026-09-08 — `status: gebaut`

`werkzeuge/kennzeichen/kennzeichen_riegel.cpp` plus its manifest, wired with `add_test` in
the shape of `bezeichner`, and the member added to `FABRIK_MITGLIEDER`. Three self-test
tables (8 / 9 / 6 cases) run before the corpus; all entry points take text, not a path.

**Two deviations, both deliberate, for the reviewer:**

1. **The corpus is `kern/src` *and* `kern/include`, not `kern/src` alone** as condition 1
   words it. Measured 2026-09-08: 57 occurrences of `abbruch(` sit in seven headers under
   `kern/include` (`zustand.hpp` 26, `festkomma.hpp` 10). An entry whose barrier lives in a
   header would have been a finding under the narrow scope without anything having rotted.
   The widening only ever *adds* places a piece may be found, and this riegel never claimed
   the piece stands in the *right* message — only that it stands in the core at all. Neither
   of today's two entries is affected: all five pieces resolve inside `kern/src`. The
   searched roots are one named list in the source, and both are printed each run.

2. **An empty Kennzeichen list, and an empty piece, are findings.** Condition 1 covers a
   piece that occurs nowhere; it does not name the case of no piece at all. An empty list
   matches every text and binds the entry to nothing — the same rot in its limit case. Both
   of today's entries carry pieces (3 and 2), so the shipped tree is unaffected.

**What I could not do:** compile. Nothing here is reachable by a reviewer's reading alone —
if the build is red, the likeliest spots are the octal `\001` escapes in the expected masks
of the first self-test table and the `std::string` / `std::string_view` comparisons, which I
wrote as explicit `std::string(...)` conversions for exactly that reason.
